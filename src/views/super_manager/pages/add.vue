  ...
  methods: {
    ...
    get_group(){
      if(!(this.add_user.role === "" ||
        this.add_user.store==="")){
        console.log(this.add_user.store)
        axios.get('api/group/list', {
          params:{
            store_id: this.add_user.store,
...
<template>
  <a-flex justify="center">
    <h2 style="font-weight: bold;font-size: 25px">增加单个用户</h2>
  </a-flex>
  <a-form
      :model="add_user"
      :label-col="{ span: 8 }"
  ...
  >
    ...
    >
      <a-input v-model:value="add_user.name"/>
    </a-form-item>
    <a-form-item
        label="所属门店"
        name="store"
    ...
    >
    <a-select v-model:value="add_user.store" @change="get_group">
      <a-select-option v-for="sto in store" :key="sto" :value="sto.store_id">
        {{sto.name}}
      </a-select-option>
    </a-select>
    </a-form-item>
